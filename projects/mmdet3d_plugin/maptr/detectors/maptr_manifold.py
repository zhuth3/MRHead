...
class MapTRManifold(MVXTwoStageDetector):
    """MapTR.
    Args:
    ...
    def __init__(self,
        ...
        # manifold_train_cfg = train_cfg.pts if train_cfg else None
        # manifold_head.update(train_cfg=manifold_train_cfg)
        # manifold_test_cfg = test_cfg.pts if test_cfg else None
        # manifold_head.update(test_cfg=manifold_test_cfg)
        self.manifold_head = builder.build_head(manifold_head)
        self.manifold_output = manifold_output
    ...
    def forward_pts_train(self,
        ...
        losses, cls_reg_targets = self.pts_bbox_head.loss(*loss_inputs, img_metas=img_metas)

        outs_manifold = self.manifold_head(outs)
        # manifold_loss_inputs = [cls_reg_targets, outs_manifold]
        manifold_loss_inputs = [gt_bboxes_3d, gt_labels_3d, outs_manifold]
        manifold_losses = self.manifold_head.loss(*manifold_loss_inputs)

        losses.update(manifold_losses)
    ...
    def simple_test_pts(self, x, lidar_feat, img_metas, prev_bev=None, rescale=False):
        ...
        if self.manifold_output:
            outs = self.manifold_head(outs)
            bbox_list = self.manifold_head.get_bboxes(
        ...
        # manifold_bbox_results = [
        #     self.pred2result(bboxes, scores, labels, pts)
        #     for bboxes, scores, labels, pts in manifold_bbox_list
        # ]
        # print('manifold_bbox_results', len(manifold_bbox_results))
        # print('pts_3d', manifold_bbox_results[0]['pts_3d'].shape)
        # assert False
        # import pdb;pdb.set_trace()
        # return manifold_outs['bev_embed'], manifold_bbox_results
