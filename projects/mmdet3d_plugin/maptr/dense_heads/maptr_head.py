...
class MapTRHead(DETRHead):
    ...
    def _get_target_single(self,
        ...
        # print('gt_bboxes: ', gt_bboxes.requires_grad)
        # print('gt_labels: ', gt_labels.requires_grad)
        # print('gt_shifts_pts: ', gt_shifts_pts.requires_grad)

        num_gt = gt_bboxes.shape[0]
        # repeat_num = int(num_bboxes/num_gt) + 1
        # gt_bboxes = gt_bboxes.repeat(2, 1)
        # gt_labels = gt_labels.repeat(2)
        # gt_shifts_pts = gt_shifts_pts.repeat(2, 1, 1, 1)
        gt_bboxes = torch.cat((gt_bboxes, gt_bboxes[:1, :]), 0).contiguous()
        gt_labels = torch.cat((gt_labels, gt_labels[:1]), 0).contiguous()
        gt_shifts_pts = torch.cat((gt_shifts_pts, gt_shifts_pts[:1, :, :, :]), 0).contiguous()
        # print('gt_bboxes: ', gt_bboxes.requires_grad)
        # print('gt_labels: ', gt_labels.requires_grad)
        # print('gt_shifts_pts: ', gt_shifts_pts.requires_grad)
        # print('gt_shifts_pts: ', gt_bboxes_ignore)
        ...
        sampling_result = self.sampler.sample(assign_result, bbox_pred,
                                              gt_bboxes)
        # print('sampling_result: ', sampling_result.info)
        # assert False
        ...
        # print('labels: ', labels[pos_inds])
        # print('bbox_targets: ', bbox_targets[pos_inds])
    ...
    def loss(self,
        ...
        for loss_cls_i, loss_bbox_i, loss_iou_i, loss_pts_i, loss_dir_i, in zip(losses_cls[:-1],
            ...
            num_dec_layer += 1
        return loss_dict, cls_reg_targets
...
# from mmdet3d.models import build_match
# from mmdet.core.bbox.assigners import AssignResult
# @HEADS.register_module()
# class MapTRGlueHead(DETRMapHead):
#     """Head of Detr3D.
#     Args:
#         with_box_refine (bool): Whether to refine the reference points
#             in the decoder. Defaults to False.
...
#         super(MapTRGlueHead, self).__init__(
#             *args, transformer=transformer, **kwargs)
#         self.code_weights = nn.Parameter(torch.tensor(
#             self.code_weights, requires_grad=False), requires_grad=False)
#         self.loss_pts = build_loss(loss_pts)
...
#         self.match = build_match(assigner['cost'])
...
#         gt_c = gt_bboxes.shape[-1]
#         # import pdb;pdb.set_trace()
#         # print('bbox_pred: ', bbox_pred.shape)#[50, 4]
#         # print('cls_score: ', cls_score.shape)#[50, 3]
#         # print('pts_pred: ', pts_pred.shape)#[50, 20, 2]
#         # print('gt_bboxes: ', gt_bboxes.shape)#[11, 4]
#         # print('gt_labels: ', gt_labels.shape)#[11]
#         # print('gt_shifts_pts: ', gt_shifts_pts.shape)#[11, 19, 20, 2]#[11, 20, 2]
#         # #print('gt_bboxes_ignore: ', gt_bboxes_ignore.shape)#[]
#         # assert False, 'test'
#         # assign_result, order_index = self.assigner.assign(bbox_pred, cls_score, pts_pred,
#         #                                      gt_bboxes, gt_labels, gt_shifts_pts,
#         #                                      gt_bboxes_ignore)
...
#         assigned_gt_inds, assigned_labels = self.match(pts_pred, gt_shifts_pts, cls_score, gt_labels)
#         assign_result, order_index = AssignResult(num_gts, assigned_gt_inds, None, labels=assigned_labels), order_index
...
#             f'for gt_bboxes_ignore setting to None.'
#         gt_vecs_list = copy.deepcopy(gt_bboxes_list)
#         # print('gt_vecs_list: ', gt_vecs_list)
#         # assert False, 'test'
#         # import pdb;pdb.set_trace()
#         all_cls_scores = preds_dicts['all_cls_scores']
#         all_bbox_preds = preds_dicts['all_bbox_preds']
...
#         gt_pts_list = [
#             gt_bboxes.fixed_num_sampled_points.to(device) for gt_bboxes in gt_vecs_list]
#         if self.gt_shift_pts_pattern == 'N':
#             gt_shifts_pts_list = [
#                 gt_bboxes.fixed_num_sampled_points.to(device) for gt_bboxes in gt_vecs_list]
#         elif self.gt_shift_pts_pattern == 'v0':
#             gt_shifts_pts_list = [
#                 gt_bboxes.shift_fixed_num_sampled_points.to(device) for gt_bboxes in gt_vecs_list]
#         elif self.gt_shift_pts_pattern == 'v1':
#             gt_shifts_pts_list = [
