...
def cosine_loss(pred, target):
    ...
    Returns:
        torch.Tensor: Calculated loss
    """
    num_samples, num_pts, num_coords = target.shape

    bbox_pred_cos = pred.view(num_samples, num_pts*num_coords)
    gt_bboxes_cos = target.view(num_samples, num_pts*num_coords)
    var = bbox_pred_cos*gt_bboxes_cos
    var = torch.sum(var.view(num_samples, num_pts, num_coords), 2)
    pred_squar = bbox_pred_cos*bbox_pred_cos
    pred_squar = torch.sqrt(torch.sum(pred_squar.view(num_samples, num_pts, num_coords), 2))
    target_squar = gt_bboxes_cos*gt_bboxes_cos
    target_squar = torch.sqrt(torch.sum(target_squar.view(num_samples, num_pts, num_coords), 2))

    loss = 1 - torch.div(var, pred_squar*target_squar)
    # print('cosine: ', loss.shape)
...
@mmcv.jit(derivate=True, coderize=True)
@custom_weighted_dir_loss
def pts_angle_loss(pred, target):
    """ Dir cosine similiarity loss
    pred (torch.Tensor): shape [num_samples, num_dir, num_coords]
    target (torch.Tensor): shape [num_samples, num_dir, num_coords]
    ...
    """
    # pred_dir = pred_dir[1:]
    # pred_dedir = pred_dedir[:-1]
    # target_dir = target_dir[1:]
    # target_dedir = target_dedir[:-1]

    # if target_dir.numel() == 0:
    #     return pred_dir.sum() * 0
    # # import pdb;pdb.set_trace()
    # num_samples, num_dir, num_coords = pred_dir.shape
    # loss_func = torch.nn.CosineEmbeddingLoss(reduction='none')
    # tgt_param = target.new_ones((num_samples, num_dir))
    # tgt_param = tgt_param.flatten(0)
    # loss_pred = loss_func(pred_dir.flatten(0,1), pred_dedir.flatten(0,1), tgt_param)
    # loss_target = loss_func(target_dir.flatten(0,1), target_dedir.flatten(0,1), tgt_param)
    # loss_pred = loss_pred.view(num_samples, num_dir)
    # loss_target = loss_target.view(num_samples, num_dir)
    loss = torch.abs(pred - target)
    return loss
...
class PtsAngleLoss(nn.Module):
    ...
    def __init__(self, reduction='mean', loss_weight=1.0):
        super(PtsAngleLoss, self).__init__()
        self.reduction = reduction
        self.loss_weight = loss_weight
    ...
    def forward(self,
        ...
        num_samples, num_dir, num_coords = pred_dir.shape
        loss_func = torch.nn.CosineEmbeddingLoss(reduction='none')
        tgt_param = target_dir.new_ones((num_samples, num_dir))
        tgt_param = tgt_param.flatten(0)
        loss_pred = loss_func(pred_dir.flatten(0,1), pred_dedir.flatten(0,1), tgt_param)
        loss_target = loss_func(target_dir.flatten(0,1), target_dedir.flatten(0,1), tgt_param)
        loss_pred = loss_pred.view(num_samples, num_dir)
        ...
        loss_angle = self.loss_weight * pts_angle_loss(
            loss_pred, loss_target, weight[1:], reduction=reduction, avg_factor=avg_factor)
        return loss_angle
...
class CosineLoss(nn.Module):
    ...
    def forward(self,
        ...
        reduction = (
            reduction_override if reduction_override else self.reduction)
        # import pdb;pdb.set_trace()
        loss_bbox = self.loss_weight * cosine_loss(
            pred, target, weight, reduction=reduction, avg_factor=avg_factor)
        return loss_bbox
...
class OrderedPtsL1Cost(object):
    ...
    def __call__(self, bbox_pred, gt_bboxes):
        ...
        """
        num_gts, num_orders, num_pts, num_coords = gt_bboxes.shape
        num_query, pred_pts, _ = bbox_pred.shape
        # import pdb;pdb.set_trace()
        bbox_pred_pt = bbox_pred.view(bbox_pred.size(0),-1)
        gt_bboxes_pt = gt_bboxes.flatten(2).view(num_gts*num_orders,-1)
        bbox_cost = torch.cdist(bbox_pred_pt, gt_bboxes_pt, p=1)

        # bbox_pred_cos = bbox_pred.view(num_query,-1)
        # gt_bboxes_cos = gt_bboxes.flatten(2).view(num_gts*num_orders,-1)
        # var = torch.mm(bbox_pred_cos, gt_bboxes_cos.T)
        # a = torch.sqrt(torch.sum(bbox_pred_cos*bbox_pred_cos, 1)).view(num_query, 1)
        # b = torch.sqrt(torch.sum(gt_bboxes_cos*gt_bboxes_cos, 1)).view(num_gts*num_orders, 1)
        # cosine_cost = torch.div(var, a+b.T)

        # bbox_cost += cosine_cost
        return bbox_cost * self.weight
...
class CosineCost(object):
    ...
    def __call__(self, bbox_pred, gt_bboxes):
        ...
        num_query, pred_pts, _ = bbox_pred.shape
        # import pdb;pdb.set_trace()
        # bbox_pred = bbox_pred.view(num_query*pred_pts,-1)
        # gt_bboxes = gt_bboxes.flatten(2).view(num_gts*num_orders*num_pts,-1)
        #bbox_cost = torch.cdist(bbox_pred, gt_bboxes, p=1)
        ...
        a = torch.sqrt(torch.sum(bbox_pred_cos*bbox_pred_cos, 1)).view(num_query, 1)
        b = torch.sqrt(torch.sum(gt_bboxes_cos*gt_bboxes_cos, 1)).view(num_gts*num_orders, 1)
        cosine_cost = 1 - torch.div(var, a+b.T)
