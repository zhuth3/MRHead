...
def get_cls_results(gen_results, 
    ...
    # print('class_id: ', class_id, 'cls_gens: ', cls_gens.shape, 'cls_gts: ', cls_gts.shape)
    # assert False
...
def eval_map(gen_results,
    ...
    for i, clsname in enumerate(cls_names):
        ...
        # print('cls_gt: ', np.array(cls_gt).shape)
        ...
        # num_det*num_res
        tp = np.cumsum(tp, axis=0)#累加
        fp = np.cumsum(fp, axis=0)
        eps = np.finfo(np.float32).eps#非零极小数
